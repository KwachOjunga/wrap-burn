...
pub mod wgpu {
    ...
    implement_wgpu_interface!(PyMultiLabelClassificationOutput,MultiLabelClassificationOutput);
    implement_wgpu_interface!(PyRegressionOutput,RegressionOutput);

    for_normal_struct_enums!(PyFileApplicationLoggerInstaller,FileApplicationLoggerInstaller);
    ...
    pub mod checkpoint {
        ...
        for_normal_struct_enums!(PyComposedCheckpointingStrategy,ComposedCheckpointingStrategy);
        for_normal_struct_enums!(PyComposedCheckpointingStrategyBuilder,ComposedCheckpointingStrategyBuilder);
        ...
        // Errors
        #[pyclass]
        struct CheckPointError{ pub inner:CheckpointerError}

        impl From<CheckPointError> for PyErr {
            fn from(error: CheckPointError) -> Self {
                match error.inner {
                    CheckpointerError::IOError(err) => PyValueError::new_err(err),
                    CheckpointerError::RecorderError(err) => PyValueError::new_err::<PyRecorderError>(err.into()),
                    CheckpointerError::Unknown(_) => PyValueError::new_err("unknown checkpoint error")
                }
            }
        }
