...
pub mod nn {
    ...
    pub mod loss {
        ...
        pub struct BinaryCrossEntropyPy {
            pub inner: burn::nn::loss::BinaryCrossEntropyLoss<NdArray>,
        }

        /// Configuration to build the BinaryCrossEntropyLoss
        #[pyclass]
        pub struct BinaryCrossEntropyConfigPy(pub burn::nn::loss::BinaryCrossEntropyLossConfig);

        /// calculate cross entropy loss from input logits to target
        #[pyclass]
        pub struct CrossEntropyLossPy {
            pub inner: burn::nn::loss::CrossEntropyLoss<NdArray>,
        }

        /// Calculate the HuberLoss between inputs and target
        #[pyclass]
        pub struct HuberLossPy(pub burn::nn::loss::HuberLoss);

        /// Configuration to build the HuberLoss
        #[pyclass]
        pub struct HuberLossConfigPy(pub burn::nn::loss::HuberLossConfig);

        /// Calculate the mean squared error loss from the input logits and the targets.
        #[pyclass]
        pub struct MseLoss(pub burn::nn::loss::MseLoss);

        /// Negative Log Likelihood (NLL) loss with a Poisson distribution assumption for the target.
        #[pyclass]
        pub struct PoissonLoss(pub burn::nn::loss::PoissonNllLoss);

        /// Configuration to calculate the PoissonLoss
        #[pyclass]
        pub struct PoissonLossConfig(pub burn::nn::loss::PoissonNllLossConfig);
